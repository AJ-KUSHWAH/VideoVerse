...
const LiveChat = () => {
  ...
  return (
    <>
      ...
      <form
        className="flex items-center gap-4 border border-gray-300 rounded-b-lg py-5 px-4 w-full border-t-0 border-r-1 border-b-1 border-l-1"
        onSubmit={(e) => {
          e.preventDefault();
          liveMessage !== "" &&
            dispatch(
      ...
      >
        <label className=" flex items-center gap-4 w-full">
          <p className=" font-bold">Ajay Kushwah</p>
          <input
            type="text"
            placeholder="Chat"
        ...
        <button className="border border-green-700 py-1 px-4 rounded-md bg-green-500 text-white font-bold">
          Send
        </button>
      </form>
    </>
