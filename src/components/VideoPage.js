...
import {
  ...
  likeCountHandler,
  likeToggle,
} from "../utilities/likeDislikeSlice";
import RecommendedVideos from "./RecommendedVideos";
import { RWebShare } from "react-web-share";
import { YOUTUBE_VIDEO_DETAILS_API_BY_ID } from "../utilities/Constants";
...
const VideoPage = () => {
  ...
  const { isLiked, likeCountStore, isDisliked } = useSelector(
    (store) => store.likeDislike
  );
  ...
  return (
    <div className="flex gap-8 mt-8 px-4 max-sm:flex-col max-sm:items-center max-sm:px-2">
      <div className="w-full max-sm:w-auto">
        ...
        <div>
          <h1 className="text-2xl font-semibold py-4 max-sm:text-lg">
            {title}
          </h1>
        </div>
            ...
            <div className="flex items-center rounded-full py-2 bg-gray-100">
              ...
              >
                ...
                <p className="text-md max-sm:text-sm">
                  {numeral(
                    likeCountStore !== 0 ? likeCountStore : likeCount
                  ).format("0.a")}
                </p>
              </div>

              <div
                className="flex items-center gap-2  py-1 px-7 max-sm:px-3"
                onClick={() => {
              ...
              >
                <i
                  className={`${
                    isDisliked ? "fa-solid" : "fa-regular"
                  } fa-thumbs-down fa-flip-horizontal fa-xl`}
                ></i>
              </div>
            </div>
